...
loadComponent('Manta2RotXFoil_AR8_b8_expt');  %Load Vehicle Model
...
wingParaMAT = {vhcl.wingRootChord.Value/10,...
...
hStabParaMAT = {vhcl.hStab.rootChord.Value/10,...
...
vStabParaSW = ["rootChord","NACAairfoil","incidentAngle",...
    "aspectRatio","taperRatio","sweepAngle"];
vStabParaMAT = {vhcl.vStab.rootChord.Value/10,...
...
fuseParaMAT = {vhcl.fuse.length.Value/10,vhcl.fuse.diameter.Value/10};
assyParaSW = ["hStabDistanceFromWing","vStabDistanceFromWing",...
    "LEDistanceFromWing"];
assyParaMAT = {vhcl.hStab.rSurfLE_WingLEBdy.Value(1)/10,...
    vhcl.vStab.rSurfLE_WingLEBdy.Value(1)/10,-vhcl.fuse.rNose_LE.Value(1)/10};
